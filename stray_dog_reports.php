...
$query = "SELECT id, description, location, photos, behaviour, status, responsible_entity, created_at FROM reportstray WHERE user_id = ? ORDER BY created_at DESC";
...
<html>
<head>
    <title>Your Stray Dog Reports</title>
...
<a href="profile.php" class="back-button">Back</a>
    ...
    <h1>Your Stray Dog Reports</h1>
    <?php if (!empty($reports)): ?>
        <?php foreach ($reports as $report): ?>
            <div class="report">
                <h2><a href="report_details.php?id=<?php echo $report['id']; ?>">Report #<?php echo $report['id']; ?></a></h2>
                ...
                <p><strong>Behaviour:</strong> <?php echo htmlspecialchars($report['behaviour']); ?></p>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <p>You have not submitted any stray dog reports.</p>
    <?php endif; ?>
    ...
    <script>
        ...
        function fetchUpdatedReports() {
            $.ajax({
                url: 'fetch_updated_reports.php', // This will call another PHP script to get the latest data
                method: 'POST',
