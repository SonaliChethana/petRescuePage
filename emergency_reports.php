...
$query = "SELECT id, description, location, photos, status, priority, responsible_entity, created_at FROM emergencyreport WHERE user_id = ? ORDER BY created_at DESC";
...
<html>
<head>
    <title>Your Emergency Reports</title>
...
<a href="profile.php" class="back-button">Back</a>
    ...
    <h1>Your Emergency Reports</h1>
    <?php if (!empty($emergency_reports)): ?>
        <?php foreach ($emergency_reports as $report): ?>
            <div class="report">
                <h2><a href="emergency_report_details.php?id=<?php echo $report['id']; ?>">Report #<?php echo $report['id']; ?></a></h2>
                ...
                <p><strong>Priority:</strong> <?php echo htmlspecialchars($report['priority']); ?></p>
                <p><strong>Responsible Entity:</strong> <?php echo htmlspecialchars($report['responsible_entity']); ?></p>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <p>You have not submitted any emergency reports.</p>
    <?php endif; ?>
    ...
    <script>
        ...
        function fetchUpdatedReports() {
            $.ajax({
                url: 'fetch_updated_emergency_reports.php', // This will call another PHP script to get the latest data
                method: 'POST',
