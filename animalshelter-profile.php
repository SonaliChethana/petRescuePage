...
if ($userRole !== 'animalShelter') {
    ...
    header("Location: animalshelter-profile.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['photos']['name'][0])) {
        ...
        $upload_dir = 'uploads/animal_shelter_photos/';
    ...
    if (!empty($_FILES['videos']['name'][0])) {
        ...
        $upload_dir = 'uploads/animal_shelter_videos/';
    ...
    $sql = "INSERT INTO animal_shelters (user_id, shelter_name, location, contact_number, description, services, open_hours, photos, videos)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('issssssss', $user_id, $shelter_name, $location, $contact_number, $description, $services, $open_hours, $photos_paths, $videos_paths);

    if ($stmt->execute()) {
        // Update user's profile to indicate that they have submitted a vet clinic form
        $update_sql = "UPDATE registration SET has_animal_shelter = TRUE WHERE id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param('i', $user_id);
        $update_stmt->execute();

        // Save vet clinic data in session to retrieve it later
        $_SESSION['animal_shelter'] = [
        ...
        echo json_encode(['success' => false, 'message' => 'Failed to save shelter information']);
    }
}
?>
...
<head>
    ...
    <title>Add your animal shelter</title>
...
<?php if (isset($_SESSION['success_message'])): ?>
    ...
    <?php endif; ?>
        ...
        <h2>Add you animal shelter</h2>
        <form action="animalshelter-profile.php" method="POST" enctype="multipart/form-data">
